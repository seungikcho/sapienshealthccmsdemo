import { MOCK_PROFILES, createEmptyProfile, type PatientProfile } from "../shared/patient-profile";
...
const SYMPTOM_KEYWORDS = [
  "fatigue", "tired", "pain", "ache", "headache", "migraine", "nausea", "dizzy",
  "dizziness", "fever", "cough", "shortness of breath", "chest tightness",
  "chest pain", "swelling", "rash", "anxiety", "insomnia", "cramp", "numbness",
  "weakness", "bleeding", "vomiting", "diarrhea",
];

/** Turn raw email text into a first-pass profile (name + scraped symptoms). */
export function buildProfileFromSearch(name: string, rawText: string): PatientProfile {
...
export function simulatePatientReply(id: string): PatientProfile | undefined {
  ...
  if (!p.symptoms?.length) p.symptoms = ["Mild recurring headache", "Occasional fatigue"];
  if (p.context == null) p.context = "follow_up";
  if (!p.pastMedicalTests?.length) p.pastMedicalTests = ["CBC (2026-03)", "Basic metabolic panel (2026-03)"];
