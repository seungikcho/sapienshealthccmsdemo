...
- [x] 사용자 제공 텍스트에서 Connect, Prioritize, Execute의 핵심 설명 문장을 추려 How it Works 카피로 재구성
- [x] How it Works 섹션을 각 단계 설명과 우측 데모 슬롯이 있는 2열 구조로 재설계
- [x] 각 단계별 데모가 들어갈 자리만 먼저 보이는 비주얼 슬롯을 구현
- [x] 전체 How it Works 섹션을 현재 다크 퍼플 원톤 디자인과 맞게 정리
- [x] pnpm check로 How it Works 리디자인 후 타입 검사를 실행
- [x] 브라우저 미리보기로 새 How it Works 설명 구조와 데모 슬롯을 검토
- [ ] How it Works 리디자인 결과를 체크포인트로 저장
- [x] Contact와 Footer의 박스형 구획을 제거하고 단일 하단 흐름 구조로 재설계
- [x] CTA와 푸터 정보를 하나의 연속된 섹션처럼 보이도록 재배치
- [x] 참고 레이아웃을 바탕으로 우리 톤에 맞는 통합 하단 디자인으로 정리
- [x] pnpm check로 하단 통합 리디자인 후 타입 검사를 실행
- [x] 브라우저 미리보기로 단일 하단 흐름 레이아웃을 검토
- [ ] 하단 통합 리디자인 결과를 체크포인트로 저장
- [ ] 메인 헤드라인을 "AI-Native Executive Assistant for Primary Care"로 변경
- [ ] 메인 헤드라인 폰트 크기를 한 단계 줄여 한눈에 읽히도록 조정
- [ ] pnpm check로 헤드라인 수정 후 타입 검사를 실행
- [ ] 브라우저 미리보기로 새 헤드라인 가독성을 검토
- [x] 현재 홈페이지 상태를 최신 헤드라인 반영본 기준으로 체크포인트 저장
- [x] 웹사이트 그림체와 어울리는 6초 제품 애니메이션 콘셉트와 장면 구성을 정리
- [x] 의사, 외부 데이터 연결, 태스크 리스트, 자동 드래프팅, AI assistant 움직임이 포함된 6초 영상 생성
- [x] 생성된 애니메이션 결과물을 검토하고 사용자에게 전달
- [ ] 현재 화면을 최종안으로 확정하고 최신 헤드라인 수정본을 체크포인트로 저장
- [x] 사용자가 제공한 의사+모니터 이미지를 고화질·투명 배경 버전으로 정리
- [x] 기존 히어로 비디오를 제거하고 새 투명 배경 비주얼로 교체
- [x] 새 히어로 비주얼에 절제된 부유/글로우 애니메이션 추가
- [x] 히어로 비주얼 교체 후 미리보기와 타입 체크를 다시 검증
- [x] 새 히어로 이미지 반영 상태를 체크포인트로 저장
- [x] 히어로 이미지를 더 고화질로 다시 정리하고 가장자리 경계를 배경에 자연스럽게 맞춤
- [x] 히어로 자산 안의 별 모양 요소를 제거하고 사이트에서도 해당 장식을 제거
- [x] 수정된 히어로 자산 반영 후 미리보기와 타입 체크를 다시 검증
- [ ] 최종 히어로 리파인 버전을 체크포인트로 저장
- [x] 히어로 UI 배열을 더 둥근 카드형 인상으로 다시 리파인
- [x] 그래픽 주변 경계가 배경과 완전히 이어지도록 사각 느낌 제거
- [x] 부드러운 톤의 새 히어로 자산 반영 후 미리보기와 타입 체크 재검증
- [ ] 연속형 히어로 그래픽 최종 버전을 체크포인트로 저장
- [x] 히어로 그래픽을 텍스트 블록 높이에 맞춰 아래로 내리고 전체 크기를 축소
- [x] 그래픽 래퍼를 더 둥글고 떠 있는 프레임 느낌으로 정리
- [x] Connect, Prioritize, Execute 라벨 박스를 그래픽 옆에 문구 없이 깔끔하게 재배치
- [x] 새 히어로 정렬안 반영 후 미리보기와 타입 체크 재검증
- [ ] 히어로 레이아웃 최종 정렬 버전을 체크포인트로 저장
- [x] 둥근 프레임과 내부 이미지의 곡률을 동일하게 맞춰 완전히 연결된 비주얼로 수정
- [x] 프레임과 이미지 사이 이질감이 없도록 마스킹·오버플로우·그림자 정리
- [x] 곡률 일치 수정 후 미리보기와 타입 체크 재검증
- [ ] 프레임-이미지 일체형 히어로 최종 버전을 체크포인트로 저장
- [x] 히어로 바깥 프레임을 제거하고 이미지 단독 구성으로 정리
- [x] 프레임 제거 후 라벨 박스 위치와 그림자 밸런스를 다시 조정
- [x] 프레임 제거안 반영 후 미리보기와 타입 체크 재검증
- [ ] 프레임 없는 히어로 최종 버전을 체크포인트로 저장
- [x] 히어로 그래픽을 더 아래로 내려 헤드라인 기준선 아래에 오도록 재배치
- [x] 그래픽 하향 이동 후 라벨 박스 수직 정렬도 함께 보정
- [x] 새 위치 반영 후 미리보기와 타입 체크 재검증
- [ ] 그래픽 하향 배치 최종 버전을 체크포인트로 저장
